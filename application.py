...
#Creating Table to store the data in user database    
db.execute("create table  if not exists users( email_id VARCHAR(20) , hash VARCHAR(30) , mobile_no VARCHAR(10))")
db.execute(" create table if not exists VACCINE(fname varchar(20),lname varchar(20),age int ,mobile_no varchar(20),email varchar(20),country varchar(20),vaccinestatus varchar(20),vaccinetype varchar(20))")
@app.route("/login", methods=["GET", "POST"])
def login():
    """Log user in to fill the form if the user is admin ,he can see the database """
...
def admin():
    #pre-defined admin username and password 
    if request.method == "POST":
        # for refresh website 
...
def register():
    """Register user and save the password using hash function"""
    if request.method == "POST":
        mobile = request.form.get("mobile_no")
        if len(mobile)!=10:
            return apology("Enter correct mobile number",400)
        email = str(request.form.get("Email"))
        #checking the email id have 314ecorp domain
...
def form():
    """ The form that collect the vaccination data """
     if request.method == "POST":
        ...
        if vaccine_status=="0-dose":
            return redirect("https://www.cowin.gov.in/home")
             ...
             return redirect("https://www.cowin.gov.in/home")
        if (vaccine_status=="2-dose" or vaccine_administered=="sputnikv"):
  ...
  # function for returning apology message using html
